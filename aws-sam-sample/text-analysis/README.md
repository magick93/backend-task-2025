...
```bash
text-analysis$ sam build --use-container
```
...
```bash
text-analysis$ sam logs -n StockCheckerFunction --stack-name "text-analysis" --tail
```
...
```bash
text-analysis$ pip install -r tests/requirements.txt --user
# unit test
text-analysis$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
text-analysis$ AWS_SAM_STACK_NAME="text-analysis" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "text-analysis"
```
