...
class StandaloneInput(BaseModel):
    ...
    baseline: conlist(Sentence, min_items=1) = Field(
        ...,
...
class ComparisonInput(BaseModel):
    ...
    comparison: conlist(Sentence, min_items=1) = Field(
        ...,
        description="Second set of sentences (comparison/target)"
    )
    
    @validator('baseline', 'comparison')
    def validate_unique_ids_within_each_list(cls, v, field):
        """Ensure all sentence IDs are unique within each list."""
        ids = [sentence.id for sentence in v]
        if len(ids) != len(set(ids)):
            raise ValueError(f'All sentence IDs must be unique within the {field.name} list')
        return v
    
    @validator('comparison')
    def validate_comparison_not_identical_to_baseline(cls, v, values):
        """Ensure comparison is not identical to baseline (optional but recommended)."""
        if 'baseline' in values:
            baseline_ids = {s.id for s in values['baseline']}
...
class ClusterInsight(BaseModel):
    ...
    Attributes:
        ...
        sentiment: Overall sentiment of the cluster (positive/negative/neutral/mixed)
...
class ComparisonCluster(BaseModel):
    ...
    @validator('baselineSentences', 'comparisonSentences')
